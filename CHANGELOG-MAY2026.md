...
Sprint de mayo 2026 enfocado en resolver deudas técnicas, mejorar estabilidad y preparar la base para fases futuras de SEVIER.
...
- Thresholds configurables via `SEVIER_INDEX_LAG_THRESHOLD` y `SEVIER_SESSION_LAG_THRESHOLD`

### #152 — Configurable thresholds
- `SEVIER_INDEX_LAG_THRESHOLD` (default: 1000)
- `SEVIER_SESSION_LAG_THRESHOLD` (default: 100)
- `SEVIER_SYNC_INTERVAL_SECS` (default: 60)

### #151 — Real test logic
- `tests/sevier_stress_test.rs` ahora tiene lógica real de stress testing
- `SessionSyncTask::new()` requiere ahora `session_store: Arc<dyn SessionStore>`
- Tests de integración con `MockSessionSyncTask` y `MockSevierClient`
...
- Confirma que `verify_save_handler` y `get_sevier_status_handler` eran el mismo endpoint
...
- #149 — Gestalt Rust → Xavier migration
...
- **Config canónica**: `config/xavier.config.json` como fuente única de runtime no-secretos, con `src/settings.rs` para deserialización + `apply_to_env()`
- **Contrato CLI/HTTP/TUI unificado**: todas las superficies usan `XAVIER_URL` con fallback al JSON config via `XavierSettings::client_base_url()`
...
- `cargo build --release` — xavier + xavier-tui compilan
...
- `release-smoke.ps1` y `release-smoke.sh`: usan `XAVIER_URL`/`XAVIER_TOKEN`, sin endpoints hardcodeados
