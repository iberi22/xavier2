...
EXIT
"@
$sqlContent | Out-File -FilePath "E:\scripts-python\xavier2\query.sql" -Encoding ascii
docker cp "E:\scripts-python\xavier2\query.sql" surrealdb:/tmp/query.sql

Write-Host ""
Write-Host "=== Try surreal sql with file input ==="
try {
    $result = docker exec surrealdb sh -c '/surreal sql --endpoint ws://localhost:8000 --namespace xavier2 --database memory --username root --password root < /tmp/query.sql 2>&1 | head -30'
...
"@
$body | Out-File -FilePath "E:\scripts-python\xavier2\query_body.json" -Encoding ascii -NoNewline
docker cp "E:\scripts-python\xavier2\query_body.json" xavier2:/tmp/query_body.json

$r3 = docker exec xavier2 sh -c 'curl -s http://surrealdb:8000/sql -X POST -H "Content-Type: application/surrealdb" -H "NS: xavier2" -H "DB: memory" -u root:root -d @/tmp/query_body.json 2>&1'
