#!/usr/bin/env python3
"""
Export and curate Xavier memory data for fine-tuning.
...
OUTPUT_FILE = Path(__file__).parent.parent / "data" / "training" / "xavier_training.jsonl"

# Categories for training data
CATEGORIES = {
    "business": ["swal", "company", "business", "product"],
    "client": ["client", "leonardo", "rodacenter", "tripro"],
    "technical": ["xavier", "cortex", "openclaw", "system", "config"],
...
def query_xavier(query: str, limit: int = 50) -> List[Dict]:
    """Query Xavier memory API."""
    import urllib.request

    data = json.dumps({"query": query, "limit": limit}).encode()
    req = urllib.request.Request(
        f"{XAVIER_URL}/memory/query",
        data=data,
        headers={
    ...
    try:
        ...
        print(f"Error querying Xavier: {e}", file=sys.stderr)
        return []

def get_all_memories() -> List[Dict]:
    """Get all memories from Xavier."""
    import urllib.request

    req = urllib.request.Request(
        f"{XAVIER_URL}/memory/list",
...
def extract_entities(content: str) -> Dict[str, List[str]]:
    ...
    products = re.findall(r'(?:ManteniApp|Cortex|Xavier|Gestalt|Synapse)', content)
...
def create_qa_pairs(doc: Dict) -> List[Dict]:
    ...
    if category == "technical":
        if "Xavier" in content or "Cortex" in content:
            pairs.append({
...
def main():
    print("=" * 60)
    print("XAVIER TRAINING DATA EXPORTER")
    print("=" * 60)

    # Get all memories
    print("\n[1/4] Fetching memories from Xavier...")
    memories = get_all_memories()
    print(f"  Found {len(memories)} memories")

    # Also query for specific topics to get more data
    print("\n[2/4] Querying for specific topics...")
    topics = ["SWAL", "ManteniApp", "Leonardo Duque", "Xavier", "Cortex", "pricing", "sales"]
    for topic in topics:
        results = query_xavier(topic, limit=10)
