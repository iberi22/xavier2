...
BASE_URL="${XAVIER_URL:-http://127.0.0.1:8006}"
TOKEN="${XAVIER_TOKEN:-}"
REQUIRE_BUILD_ROUTE="${XAVIER_REQUIRE_BUILD_ROUTE:-0}"
REQUIRE_PANEL="${XAVIER_REQUIRE_PANEL:-0}"
...
if [[ -z "${TOKEN}" ]]; then
  echo "XAVIER_TOKEN is required for release smoke checks" >&2
  exit 1
fi

echo "Running Xavier release smoke checks against ${BASE_URL}"
...
[[ "${readiness}" == *'"service":"xavier"'* ]]
echo "PASS /readiness"

build_status="$(curl -s -o /tmp/xavier-build-smoke.json -w "%{http_code}" \
  -H "X-Xavier-Token: ${TOKEN}" \
  "${BASE_URL}/build")"
if [[ "${build_status}" == "200" ]]; then
  build="$(cat /tmp/xavier-build-smoke.json)"
  [[ "${build}" == *'"service":"xavier"'* ]]
  echo "PASS /build"
...
if [[ "${REQUIRE_PANEL}" == "1" ]]; then
  ...
  panel_chat="$(curl -fsS \
    ...
    -H "X-Xavier-Token: ${TOKEN}" \
    -H "Content-Type: application/json" \
    -d "{\"thread_id\":\"${thread_id}\",\"message\":\"Explain xavier memory and show a structured UI.\"}")"
...
echo "Xavier release smoke checks passed."
