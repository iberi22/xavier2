# Xavier2 - Windows Installer
# Ejecutar en PowerShell como Administrador

$ErrorActionPreference = "Stop"
$XavierDir = "$env:LOCALAPPDATA\Xavier2"
$BinDir = "$XavierDir\bin"
$BinPath = "$BinDir\xavier2.exe"
$DataDir = "$XavierDir\data"
$ConfigDir = "$XavierDir\config"
$SourceExe = "$PSScriptRoot\target\release\xavier2.exe"

Write-Host "=== Xavier2 Windows Installer ===" -ForegroundColor Cyan

# 1. Verificar binario
if (-not (Test-Path $SourceExe)) {
    Write-Host "ERROR: No se encuentra target\release\xavier2.exe" -ForegroundColor Red
    Write-Host "Ejecuta primero: cargo build --release --bin xavier2 -j 1" -ForegroundColor Yellow
    exit 1
}
...
Write-Host "  xavier2.exe -> $BinPath ($((Get-Item $BinPath).Length / 1MB) MB)" -ForegroundColor Green
...
# 6. Setear XAVIER2_TOKEN si no existe
$CurrentToken = [Environment]::GetEnvironmentVariable("XAVIER2_TOKEN", "User")
if (-not $CurrentToken) {
    $NewToken = -join ((65..90) + (97..122) + (48..57) | Get-Random -Count 32 | % { [char]$_ })
    [Environment]::SetEnvironmentVariable("XAVIER2_TOKEN", $NewToken, "User")
    Write-Host "  XAVIER2_TOKEN generado y seteado" -ForegroundColor Green
    
    # También actualizar en el .env si existe
    if (Test-Path $ConfigDest) {
        $EnvContent = Get-Content $ConfigDest -Raw
        $EnvContent = $EnvContent -replace "XAVIER2_TOKEN=.*", "XAVIER2_TOKEN=$NewToken"
...
Write-Host ""
Write-Host "Ejecutar: xavier2 --help" -ForegroundColor Cyan
Write-Host "Servidor: xavier2 serve" -ForegroundColor Cyan
Write-Host "Modo dev:  xavier2 monitor" -ForegroundColor Cyan
