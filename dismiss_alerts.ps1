...
$alerts = Invoke-RestMethod -Uri "https://api.github.com/repos/iberi22/xavier/dependabot/alerts" -Headers $headers -Method Get
$alerts | Where-Object { $_.state -eq 'open' -and $_.security_advisory.vulnerabilities[0].severity -eq 'low' } | ForEach-Object {
    ...
    try {
        ...
        Invoke-RestMethod -Uri "https://api.github.com/repos/iberi22/xavier/dependabot/alerts/$num" -Headers $headers -Method PUT -ContentType "application/vnd.github+json" -Body $body
    } catch {
