...
**Created:** 2026-04-05
**Purpose:** Evaluate Xavier2 memory system against real SWAL operational use cases
...
This benchmark evaluates the Xavier2 memory system using real SWAL operational queries derived from actual agent sessions, memory files, and project data. Unlike synthetic benchmarks, SWAL-LoCoMo uses genuine operational questions that BELA and the ventas agent encounter daily.
...
- **Xavier2 v0.4.1**: Running with FileMemoryStore (SurrealDB disabled due to protocol mismatch)
- **pplx-embed**: Healthy, 1024d embeddings
- **HyDE**: Disabled (XAVIER2_DISABLE_HYDE=1)
...
| SH-05 | What is Xavier2's current version? | BENCHMARKS.md → v0.4.1 |
...
|----|-------|---------------------|
| MH-01 | Who worked on Xavier2 fixes and what decisions were made? | 2026-04-05.md + MEMORY.md connections |
...
| MH-06 | What is the Xavier2 architecture for memory? | memory-store.json → schema, bridge, stores |
...
| TR-03 | What happened in the last Xavier2 session? | 2026-04-05.md → Full night of fixes |
| TR-04 | When was the last security audit? | 2026-03-31.md → 8 AM daily |
| TR-05 | What is the timeline of Xavier2 versions? | BENCHMARKS.md → v0.4.0 (failed), v0.4.1 (current) |
...
| ID | Query | Requires |
|----|-------|----------|
| OD-01 | Summarize the Xavier2 memory system improvements made | BENCHMARKS.md + 2026-04-05.md synthesis |
...
| OD-05 | What is the complete SWAL product portfolio? | MEMORY.md → Xavier2, ManteniApp, Software Factory, etc. |
...
- JSON: `E:\scripts-python\xavier2\benchmark-results\swal-locomo-{date}.json`
- Markdown: `E:\scripts-python\xavier2\BENCHMARKS\SWAL_LOCOMO_BASELINE_{date}.md`
