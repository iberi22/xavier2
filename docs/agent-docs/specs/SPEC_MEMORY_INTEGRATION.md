---
title: "Memory Integration Specification (GitHub Issues vs. Xavier)"
type: SPECIFICATION
...
summary: |
  ...
  long-term repository memory (Xavier MCP/HTTP).
keywords: [memory, xavier, github, issues, state, rag]
...
| **Project Knowledge** | Xavier (MCP/HTTP) | Permanent | Architecture, research, complex logic, past solutions. |

## 3. Usage Patterns

### A. Researching a Task (Read)
Before starting a new issue, agents **MUST**:
1. Search Xavier for similar past implementations: `search_memories(query: "...")`.
...
1. Store reusable findings in Xavier: `add_memory(path: "research/...", content: "...")`.
2. Link to the resolving PR or Issue in the memory metadata.
3. **DO NOT** store temporary checklists or small debugging logs in Xavier.
...
- References to Xavier entry IDs (e.g., `xavier_ref: "doc-1234"`).
- Immediate, non-reusable state.

## 5. Tooling

- **MCP Tool**: `xavier-memory` (Primary for IDE-native agents).
