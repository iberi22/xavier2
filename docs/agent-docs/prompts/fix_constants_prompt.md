You are at E:\scripts-python\xavier on branch fix/issue-115.
...
impl WorkingMemoryLayerConfig {
    pub fn from_env() -> Self {
        ...
        Self {
            capacity: std::env::var("XAVIER_WORKING_MEMORY_CAPACITY")
                .ok().and_then(|v| v.parse().ok()).unwrap_or(default.capacity),
            lru_exempt_access_threshold: std::env::var("XAVIER_WORKING_LRU_THRESHOLD")
                .ok().and_then(|v| v.parse().ok()).unwrap_or(default.lru_exempt_access_threshold),
            bm25_k1: std::env::var("XAVIER_WORKING_BM25_K1")
                .ok().and_then(|v| v.parse().ok()).unwrap_or(default.bm25_k1),
            bm25_b: std::env::var("XAVIER_WORKING_BM25_B")
...
cargo test --lib tools_list_returns_all_xavier_tools 2>&1 | tail -5
