# Xavier2 / Cortex / Engram — Architecture

> **Goal:** Stable, dogfooded, green. Xavier2 as open core, Cortex as enterprise layer, Engram as design reference only.
...
### Xavier2 — Core Open Source Engine
- **Role:** Primary memory and code graph engine
- **Repo:** `E:\scripts-python\xavier2`
- **Binary:** `C:\Users\belal\.cargo\target_global\release\xavier2.exe`
- **Protocols:** HTTP (:8006), MCP-stdio, CLI
- **Storage:** `xavier2_memory_vec.db` (SQLite + vec)
- **Build:** `cargo build --release -p xavier2 --bin xavier2`
- **Check:** `cargo check -p xavier2 --bin xavier2` (exit 0, zero warnings)
...
- **Script:** `E:\scripts-python\xavier2\scripts\memory_triad_loop.ps1`
...
- **Fix needed:** Align embedding model dimensions between xavier2 (1024) and cortex vec store (768)
...
1. **Xavier2 is the open core** — public, community-facing, simple deploy
...
5. **Dogfood first** — SWAL uses Xavier2 for its own code/graph before selling it